...
(function run(){
  ...
  const base = get(DEMO_MAIL_API_URL) || get(DEMO_STALWART_API_URL) || (get(STALWART_URL)? get(STALWART_URL).replace(/\/$/,)+/api : https://mailserver.example.com/api);
  const token = get(DEMO_MAIL_API_TOKEN) || get(STALWART_API_KEY_AUTH_BEARER_TOKEN);
  ...
  const intDom = get(DEMO_INTERNAL_EMAIL_DOMAIN,workinpilot.space);
  ...
  (async()=>{
    try{
      ...
      process.env.DEMO_MAIL_API_URL=base;
      process.env.DEMO_MAIL_API_TOKEN=token;
