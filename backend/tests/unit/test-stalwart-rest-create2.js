...
(async () => {
  try {
    ...
    // Check response body for errors (Stalwart returns 200 with error in body for OIDC directory)
    const hasError = createRes.data && (createRes.data.error || createRes.data.errors);
    const errorMessage = createRes.data?.error || createRes.data?.details || '';
    
    // Interpret results
    if (hasError) {
      if (errorMessage.includes('OpenID directory') || errorMessage.includes('cannot be managed')) {
        console.log(`\nFAIL OIDC DIRECTORY BLOCKS REST API CREATION`);
        console.log(`   Error: ${errorMessage}`);
        console.log(`   Conclusion: You MUST use OIDC pre-deploy (OAuth flow), not REST API create`);
        console.log(`   The verifyAndEnableMailbox() approach will NOT work for OIDC directory`);
      } else {
      ...
      if (fetchRes.status === 200 && !fetchRes.data?.error) {
      ...
      console.log(`   Request format may be wrong`);
